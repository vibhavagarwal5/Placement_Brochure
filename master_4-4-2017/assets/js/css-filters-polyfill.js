	...
	var polyfilter = {
		...
		_create_svg_element: function(tagname,attributes){
			...
			for(key in attributes){
				elem.setAttributeNS( ,key,attributes[key]);
			}
		...
		_create_svg:	function(id,filterelements){
			...
			svg.setAttributeNS( ,'width','0');
			svg.setAttributeNS( ,'height','0');
			svg.setAttributeNS( ,'style','position:absolute');
			
			var svg_filter = document.createElementNS(xmlns,'filter');
			svg_filter.setAttributeNS( ,'id',id);
		...
		process:		function(){
			...
			for(var i = 0; i < this._stylesheets.length; i++){
				...
				if(sheet !==  ) for(var j in sheet.cssRules){
